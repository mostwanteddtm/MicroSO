...
ReadSectors:
     ...
     .SUCCESS:
          ...
          pop     bx
          pop     ax
          add     bx, [bpbBytesPerSector]               ; [512] queue next buffer
...
ClusterLBA:
          ...
          xor     cx, cx
          mov     cl, [bpbSectorsPerCluster]            ; [1] convert byte to word
          mul     cx
...
LBACHS:
          ...
          div     [bpbSectorsPerTrack]  ; [18d] calculate
          inc     dl                    ; adjust for sector 0
          mov     [absoluteSector], dl
          xor     dx, dx                ; prepare dx:ax for operation
          div     [bpbHeadsPerCylinder] ; [2] calculate
          mov     [absoluteHead], dl
...
main:
     ...
     ; compute size of root directory and store in "cx"
          ...
          mul     [bpbRootEntries]    ; [224d] total size of directory
          div     [bpbBytesPerSector] ; [512d] sectors used by directory
          xchg    ax, cx
          
     ; compute location of root directory and store in "ax"
     
          mov     al, [bpbNumberOfFATs]    ; [2] number of FATs
          mul     [bpbSectorsPerFAT]       ; [9] sectors used by FATs
          add     ax, [bpbReservedSectors] ; [1] adjust for bootsector
          mov     [datasector], ax         ; base of root directory
          add     [datasector], cx
          
     ; read root directory into memory (0700:0200)
     
          mov     bx, 0x0300
          mov     cx, 0x1                            ; copy root dir above bootcode
          call    ReadSectors
     ...
     ; browse root directory for binary image
          mov     cx, [bpbRootEntries]         ; [224d] load loop counter
          mov     di, 0x0300                   ; locate first root entry
     .LOOP:
          push    cx
     ...
     ; compute location of FAT and store in "ax"
          ...
          mov     ax, [bpbReservedSectors]  ; [1] adjust for bootsector
          
     ; read FAT into memory (7C00:0200)

          mov     bx, 0x0300 
          mov     cx, 0x1                         ; copy FAT above bootcode
          call    ReadSectors
     ...
     ; read image file into memory (0800:0000)
          ...
          mov     bx, 0x0400                           ; destination for image
          push    bx
     ...
     LOAD_IMAGE:  
          ...
          xor     cx, cx
          mov     cl, [bpbSectorsPerCluster]            ; [1] sectors to read
          call    ReadSectors
          push    bx 
     ...
     ; compute next cluster 
          ...
          mov     bx, 0x0300                            ; location of FAT in memory
